...
public class Game {
	...
	public double lastTimePoint() {
		...
		while(it1.hasNext()) {
			...
			Iterator<PathPoint> it2 = temp_Packman.GetPathpoint().iterator();
			PathPoint temp_PathPoint ;
			while(it2.hasNext()) {
				temp_PathPoint=(PathPoint)it2.next();
				if(temp_PathPoint.getTime()>lastTime) {
					lastTime=temp_PathPoint.getTime();
				}
